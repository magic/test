...
/**
  ...
  */
 export function resolve(...args: [pathname: string] | [routeId: string, params: Record<string, string>]): string {
   if (args.length === 1) {
     const pathname = args[0] as string;
     if (typeof pathname === 'string' && (pathname.startsWith('http://') || pathname.startsWith('https://'))) {
       return pathname;
     }
