...
var view = C('view');
var initialState = {
  'description': ['simple tests with lots of utility. ecmascript modules only.', 'runs ecmascript module tests without transpilation.', 'unbelievably fast.'],
  'gdpr': {
    'allowed': [],
    'show': true
  },
  ...
  'menu': [{
    ...
    'items': [{
      ...
      'text': 'multiple tests in one file',
      'to': '-multiple'
    }, {
      'text': 'promises',
      'to': '-promises'
...
var pages = {
  '/test/': function test(state) {
    return [h1(state.title), state.description.map(function (d) {
      ...
      id: 'tests-cb'
    }, 'callback functions'), Pre("\nimport { promise, is } from '@magic/test'\n\nconst fnWithCallback = (err, arg, cb) => cb(err, arg)\n\nexport default [\n  {\n    fn: promise(cb => fnWithCallback(null, true, cb)),\n    expect: true\n    info: 'handle callback functions as promises',\n  },\n  {\n    fn: promise(cb => fnWithCallback(new Error('oops'), true, cb)),\n    expect: is.error,\n    info: 'handle callback function error as promise',\n  },\n]"), h3({
      id: 'tests-hooks'
    }, 'run functions before and/or after individual test'), Pre("\nconst after = () => {\n  global.testing = 'Test has finished, cleanup.'\n}\n\nconst before = () => {\n  global.testing = false\n\n  // if a function gets returned,\n  // this function will be executed once the test finished.\n  return after\n}\n\nexport default [\n  {\n    fn: () => { global.testing = 'changed in test' },\n    // if before returns a function, it will execute after the test.\n    before,\n    after,\n    expect: () => global.testing === 'changed in test',\n  },\n]"), h3({
      id: 'tests-suite-hooks'
    }, 'run functions before and/or after a suite of tests'), Pre("\nconst afterAll = () => {\n  // Test has finished, cleanup.'\n  global.testing = undefined\n}\n\nconst beforeAll = () => {\n  global.testing = false\n\n  // if a function gets returned,\n  // this function will be executed once the test suite finished.\n  return afterAll\n}\n\nexport default [\n  {\n    fn: () => { global.testing = 'changed in test' },\n    // if beforeAll returns a function, it will execute after the test suite.\n    beforeAll,\n    // this is optional and can be omitted if beforeall returns a function.\n    // in this example, afterAll will trigger twice.\n    afterAll,\n    expect: () => global.testing === 'changed in test',\n  },\n]"), h2({
      id: 'lib'
    }, 'Utility Belt'), p('@magic/test exports some utility functions that make working with complex test workflows simpler.'), h4({
      id: 'lib-curry'
    }, 'curry'), p('Currying can be used to split the arguments of a function into multiple nested functions.'), p(['This helps if you have a function with complicated arguments', ' that you just want to quickly shim.']), Pre("\nimport { curry } from '@magic/test'\n\nconst compare = (a, b) => a === b\nconst curried = curry(compare)\nconst shimmed = curried('shimmed_value')\n\nexport default {\n  fn: shimmed('shimmed_value'),\n  expect: true,\n  info: 'expect will be called with a and b and a will equal b',\n}\n"), h4({
      id: 'lib-vals'
    }, 'vals'), p(['exports some javascript types. more to come.', ' will sometime in the future be the base of a fuzzer.']), h3({
      id: 'lib-promises'
    }, 'promises'), p('Helper function to wrap nodejs callback functions and promises with ease.'), p('Handles the try/catch steps internally and returns a resolved or rejected promise.'), Pre("\nimport { promise, is } from '@magic/test'\n\nexport default [\n  {\n    fn: promise(cb => setTimeOut(() => cb(null, true), 200)),\n    expect: true,\n    info: 'handle promises in a nice way',\n  },\n  {\n    fn: promise(cb => setTimeOut(() => cb(new Error('error')), 200)),\n    expect: is.error,\n    info: 'handle promise errors in a nice way',\n  },\n]"), h3({
      id: 'lib-css'
    }, 'css'), p(['exports ', Link({
      to: 'https://github.com/magic/css'
    }, '@magic/css'), ' which allows parsing and stringification of css-in-js objects.']), h4({
      id: 'lib-trycatch'
    }, 'tryCatch'), p('allows to test functions without bubbling the errors up into the runtime'), Pre("\nimport { is, tryCatch } from '@magic/test'\n\nconst throwing = () => throw new Error('oops')\nconst healthy = () => true\n\nexport default [\n  {\n    fn: tryCatch(throwing()),\n    expect: is.error,\n    info: 'function throws an error',\n  },\n  {\n    fn: tryCatch(healthy()),\n    expect: true,\n    info: 'function does not throw'\n  },\n]"), h2({
      id: 'usage'
    }, 'Usage'), h3({
      id: 'usage-js'
    }, 'mjs api:'), Pre("\n// test/index.mjs\nimport run from '@magic/test'\n\nconst tests = {\n  lib: [\n    { fn: () => true, expect: true, info: 'Expect true to be true' }\n  ],\n}\n\nrun(tests)"), h2({
      id: 'usage-cli'
    }, 'cli'), h3('package.json (recommended)'), p('Add the magic/test bin scripts to package.json'), Pre("\n{\n  \"scripts\": {\n    \"test\": \"t -p\",\n    \"coverage\": \"t\",\n    \"format\": \"f -w\",\n    \"format:check\": \"f\"\n  },\n  \"devDependencies\": {\n    \"@magic/test\": \"github:magic/test\"\n  }\n}"), p('then use the npm run scripts'), Pre("\nnpm test\nnpm run coverage\nnpm run format\nnpm run format:check"), h3({
      id: 'usage-global'
    }, 'Globally (not recommended):'), p(['you can install this library globally', ' but the recommendation is to add the dependency and scripts to the package.json file.']), p(['this both explains to everyone that your app has these dependencies', ' as well as keeping your bash free of clutter']), Pre("\nnpm i -g magic/test\n\n// run tests in production mode\nt -p\n\n// run tests and get coverage in verbose mode\nt\n\n// check formatting using prettier but do not write\n// prettier --list-different\nf\n\n// format files using prettier\n// prettier --write\nf -w"), p(['This library tests itself, have a look at ', Link({
      to: 'https://github.com/magic/test/tree/master/test'
    }, 'the tests')]), p('Checkout [@magic/types](https://github.com/magic/types) and the other magic libraries for more test examples.')];
  },
  '/test/404/': function test404() {
